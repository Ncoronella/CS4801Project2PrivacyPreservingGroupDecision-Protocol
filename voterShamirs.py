...
class Voter:
    ...
    def castVote(self, vote: int):
        ...
        return commitment, voteShares, proof
    ...
    def receiveShareAndVerify(self, senderId, share, proof,commit):
        # ensure 1 share per sender
        if senderId not in self.receivedVoteShares:

            if not self.verifyVoteProof(commit, proof):
            ...
            # if not self.verifyShare(share, commit):
...
for i, v in enumerate(votes):
    comm, shards, proof = voters[i].castVote(v)
...
# This would happen over a secure channel directly between voters (RSA)
    for destID in range(numVoters):
            is_valid = voters[destID].receiveShareAndVerify(i, shards[destID], proofs[i], publicCommitments[i])
